...
from app.agents.config import bedrock_model, nova_sonic_model
...
class SimpleVoiceOrderingAgent:
    """Simplified voice ordering agent that works without Pipecat for basic functionality."""
    
    def __init__(self, business_id: str, use_voice_model: bool = False):
        ...
        # Choose model based on usage (voice vs text demo)
        model = nova_sonic_model if use_voice_model else bedrock_model
...
When the customer seems ready to order, guide them through the process step by step."""
    ...
    async def process_voice_input(self, audio_input: str) -> str:
        ...
        try:
            response = await self.agent.invoke_async(audio_input)
            return response.message.content[0].text if response.message.content else "I'm sorry, I couldn't process that."
        except Exception as e:
